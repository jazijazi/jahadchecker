...
class Command(BaseCommand):
    help = 'Sync user accessible contracts from layer permissions'
    ...
    def handle(self, *args, **options):
        if options['user_id']:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Synced Contracts for user {user.username}')
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully synced Contracts for all {total} users')
            )